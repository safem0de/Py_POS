...
def read_heroes(
    *,
    ...
    heroes = session.exec(select(s.Hero).offset(offset).limit(limit)).all()
    return heroes
...
def update_hero(
    *, session: Session = Depends(get_session), hero_id: int, hero: s.HeroUpdate
    ...
    db_hero = session.get(s.Hero, hero_id)
    if not db_hero:
        raise HTTPException(status_code=404, detail="Hero not found")
...
def read_teams(
    *,
    ...
    teams = session.exec(select(s.Team).offset(offset).limit(limit)).all()
    return teams
...
def update_team(
    *,
    ...
    db_team = session.get(s.Team, team_id)
    if not db_team:
        raise HTTPException(status_code=404, detail="Team not found")
